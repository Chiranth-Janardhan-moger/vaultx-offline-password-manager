...
- Visual wizard rendering steps 0-3 with dot indicators:
  ...
  - **VPin Setup (Step 2)**: Visual track of 6 PIN digits boxes. Tapping triggers hidden TextInput (number-pad). Fills boxes with dot characters `●` and outlines the active index box. Confirms matching PIN input.
...
- **Dual-Action Floating Action Button (FAB)**:
  ...
  - Displays slide-up card layout showing options:
    ...
    2. *Generate New Password* (Ionicon `sparkles`, purple background, opens `/generate-password`; warns if master password is unset).
...
- Header search bar with query highlighting in search result strings.
- Favorites filter toggle chip (`star` icon) displaying starred credentials only.
...
- **Backup Export (`app/export.tsx`)**:
  - User creates a backup password (min 6 chars).
  - Decrypts the local database and retrieves the Master Password.
  - Encrypts the passwords list and the Master Password with AES-256 using the *backup password*.
  - Packages the output JSON (version "2.0") and writes it to a `.vxb` file before opening the system sharing sheet.
- **Backup Import (`app/import.tsx`)**:
  - Dashed card interface opening `DocumentPicker` to select a backup `.vxb` file.
  - User inputs the backup password to decrypt the payload.
  - Validates version format. Merges credentials list, updates session databases, and restores the Master Password wrapper state in SecureStore.
