...
# Fetch configuration from environment variables (Secrets)
...
# Target files to upload passed via command line arguments
...
def create_or_update_release(links):
    ...
    release_data = {
        ...
        "tag_name": tag_name,
        "description": f"Release version {tag_name} built securely via GitHub Actions CI/CD pipeline.\n\n### SHA256 Checksums\nVerify downloaded assets locally using generated checksum files.",
        "assets": {
            "links": links
        }
    }
    ...
    try:
        ...
        err_msg = e.read().decode('utf-8')
        # If release already exists (409 Conflict), try updating it
        if e.code == 409:
            print(f"Release for tag '{tag_name}' already exists. Updating release assets instead...")
            update_url = f"{release_url}/{tag_name}"
            # Update API does not directly allow appending assets easily in one payload,
            # so we modify release description and update the assets individually or rely on updating description.
            # In GitLab, to append links, we create release links via the Release Links API:
...
for file_path in files_to_upload:
    ...
    if upload_res:
        # Construct absolute download URL from GitLab response
